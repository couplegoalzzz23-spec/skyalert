...
# Mematikan peringatan SSL (Server BMKG sering bermasalah dengan SSL yang membuat request gagal)
urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)

# =====================================================
# CONFIG & STYLE (DESAIN ASLI ANDA)
# =====================================================
st.set_page_config(
...
# FUNGSI UNDUH GAMBAR DIPERKUAT (ANTI TIMEOUT & BYPASS SSL)
@st.cache_data(ttl=300)
def fetch_image(wmo):
    urls = [
    ...
    for url in urls:
        try:
            # verify=False mencegah error SSL BMKG, timeout dinaikkan ke 15 detik
...
# FUNGSI EKSTRAKSI OCR (MEMBACA GAMBAR YANG SUDAH DIUNDUH)
def extract_rason_via_ocr(img):
    try:
        import pytesseract
        # Arahkan ke lokasi instalasi Tesseract di Windows Anda
        pytesseract.pytesseract.tesseract_cmd = r'C:\Program Files\Tesseract-OCR\tesseract.exe'
        
        w, h = img.size
        # Potong bagian atas gambar agar mesin fokus pada teks (hindari grafik)
        ...
        if c or k or l:
            ...
            return {"is_live": False, "error": "Teks resolusi buram/tidak terbaca mesin"}
            
    except Exception as e:
        if "tesseract is not installed" in str(e).lower() or "tesseract_cmd" in str(e).lower():
            return {"is_live": False, "error": "Aplikasi Tesseract OCR belum terinstal di Windows Anda."}
        return {"is_live": False, "error": f"Error Mesin: {e}"}
...
# --- PROSES PENGAMBILAN DATA REAL-TIME (GAMBAR & OCR) ---
# 1. Unduh gambar 1 kali saja untuk menghemat waktu dan mencegah timeout
skewt_img, img_timestamp = fetch_image(row["wmo"])

rason_live = {"is_live": False}
if skewt_img:
    # 2. Jika gambar berhasil didapat, minta mesin OCR membacanya
    rason_live = extract_rason_via_ocr(skewt_img)

# 3. Logika Penentuan Angka
if rason_live.get("is_live"):
    ...
    data_source_msg = f"🟢 **Real-Time Data Valid:** Diekstrak langsung via OCR dari diagram Skew-T BMKG (WMO {row['wmo']})."
else:
    c_val, k_val, l_val, f_val, w_val = row["cape"], row["ki"], row["li"], row["freeze"], row["wind"]
    if skewt_img:
        err_msg = rason_live.get("error", "Gagal mengenali teks.")
        data_source_msg = f"🟠 **Peringatan Fallback:** Gambar BMKG berhasil diunduh, namun mesin gagal mengekstrak angka ({err_msg}). Menampilkan database statis."
    else:
        data_source_msg = f"🔴 **Peringatan Fallback:** Gagal mengunduh gambar dari server BMKG (Server Down/Timeout). Menampilkan database historis statis."
...
# Keterangan transparansi sumber data
st.caption(f"📊 Sumber Data: {data_source_msg} Referensi visual dapat dilihat di [BMKG Monitoring Radiosonde](https://aviation.bmkg.go.id/monitoring_rason/index).")
...
if skewt_img:
    ...
    st.image(skewt_img, use_container_width=True)
else:
